...
export const Teresa = {
  ...
  pourRM(diff) {
    if (this.pouredAmount>= 1e300) {return} ;
    ...
    this.pouredAmount += Math.min(rmPoured,1e300);
  ...
  get fill() {
    return Math.min(Math.log10(this.pouredAmount) / 24, 9999);
  },
  get possibleFill() {
    return Math.min(Currency.realityMachines.value.plus(this.pouredAmount).log10() / 24, 1);
  },
  get rmMultiplier() {
    if (this.pouredAmount >= 1e24) {return this.pouredAmount**0.1}
