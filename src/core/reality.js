...
export const GlyphSelection = {
  ...
  get choiceCount() {
    return (Effects.max(1,Perk.firstPerk) +Perk.moreChoice.effectOrDefault(0))*
...
function giveRealityRewards(realityProps) {
  ...
  const realityAndPPMultiplier = Math.round((multiplier + binomialDistribution(multiplier, Achievement(154).effectOrDefault(0))));
  ...
  Currency.realities.add(realityAndPPMultiplier*SecretAchievement(11).effectOrDefault(1));
  Currency.perkPoints.add(realityAndPPMultiplier*TimeStudy(261).effectValue);
...
export function updateGL() {
  for (const GL of player.reality.glyphs.active) {
     if (Perk.GLupd.isBought) {
        if (GL.level < gainedGlyphLevel().actualLevel) {
     GL.level = gainedGlyphLevel().actualLevel;
     }
   }
  }
  for (const GL of player.reality.glyphs.inventory) {
      if (Perk.GLupd.isBought) {
        if (GL.level < gainedGlyphLevel().actualLevel) {
     GL.level = gainedGlyphLevel().actualLevel;
     }
...
export function RSgained() {
  return realityProps.gainedShards
}