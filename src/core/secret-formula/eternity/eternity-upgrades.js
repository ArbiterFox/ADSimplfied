...
export const eternityUpgrades = {
  ...
  tdMultRealTime: {
    ...
    description: () => (Pelle.isDoomed
      ? "Time Dimensions are multiplied by days played in this Armageddon and news tickers seen"
      : "Time Dimensions are multiplied by days played and news tickers seen"
    ),
    effect: () => (Pelle.isDoomed ? 1 + Time.thisReality.totalDays*player.news.totalSeen : Math.max(Time.totalTimePlayed.totalDays*player.news.totalSeen,player.news.totalSeen+1)),
    formatEffect: value => formatX(value, 2, 1)
  }
};
