...
export const normalTimeStudies = [
  {
    ...
    description: "Continuum gives 5% more extra purchases",
    effect: 1.05
  },{
    id: 11,
    cost: 1,
  ...
  },
  {
    ...
    requirement: [11],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `Replicanti is faster based on distance from e308 (2x at least)`,
    effect: () => Math.round(Math.max(308-player.replicanti.amount.exponent,3)**0.5),
  ...
  },
  {
    ...
    of your Infinities as Banked Infinities,increased by your infinities`,
    effect: () => Currency.infinities.value.times(0.05+0.01*Math.max(player.infinities.log10(),0)).floor(),
    formatEffect: () => `+${formatPercents(0.05+0.01*Math.max(player.infinities.log10(),1), 3)}`
  },
  {
    id: 51,
    cost: 3,
    requirement: [41],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: `You gain more Infinities and etenities based on Galaxies`,
    effect: () => Math.max(Math.pow(Replicanti.galaxies.total + player.galaxies + player.dilation.totalTachyonGalaxies,1.05), 1),
    formatEffect: value => formatX(value, 2)
  },
  {
    id: 52,
    ...
    requirement: [41, 42],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `You gain ${formatX(1e15)} more Infinity Points,more based upon infinities`,
    effect: () => Math.pow(1e15,Math.max(player.infinities.log10()+1,1)**0.3),
    formatEffect: value => formatX(value, 2),
  },
  {
    id: 61,
  ...
  },
  {
    ...
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    requiresST: [121, 123],
    description: () => (`You gain more Eternity Points based on time spent this Eternity and How fast your last 10 eternities are`),
    effect: () => (Perk.studyPassive.isBought ? (50*Math.pow(Time.thisEternity.totalMilliseconds,0.7))**0.5 : (Math.clamp(250 / Player.averageRealTimePerEternity, 1, 50)*Math.pow(Time.thisEternity.totalMilliseconds,0.5))**0.5),
  ...
  },
  {
    ...
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    requiresST: [131, 133],
    description: () => (`Replicanti Galaxies are ${formatPercents(0.3)} stronger,and you can get ${formatPercents(0.3)} more`),
    effect: 0.3
  },
  {
    id: 133,
  ...
  },
  {
    ...
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    requiresST: [141, 143],
    description: () => `Multiplier to Infinity Points, which both decays and increases over this Infinity`,
    effect: () => {
      const ACTeffect = DC.E45.divide(thisInfinityMult(Time.thisInfinity.totalSeconds**1.3)).clampMin(1);
      const totalSeconds = Time.thisInfinity.totalSeconds;
      return thisInfinityMult(totalSeconds).times(ACTeffect);
    },
    formatEffect: value => formatX(value, 2, 1),
  ...
  },
  {
    ...
    requirement: [152],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `multiplier to Eternities based on current eternities`,
    effect: () => Math.max(Currency.eternities.value.log10()**1.5,1),
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 152,
    ...
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `${formatX(1e4)} multiplier on all Time Dimensions,boosted by total EC tiers completed`,
    effect: () => Math.pow(1e4,1+0.1*EternityChallenges.completions**0.7),
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 153,
    ...
    requirement: [152],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `tickspeed provide minor boost to replicanti speed`,
    effect: () => Math.max(Math.log10(Math.max(Tickspeed.perSecond.log10(),1)),1),
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 161,
    ...
    requirement: [152],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `${formatX(DC.E616)} multiplier on all Antimatter Dimensions,boosted by total EC tiers completed`,
    effect: () => DC.E616.pow(1+0.1*EternityChallenges.completions**0.7) ,
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 162,
    ...
    requirement: [152],
    reqType: TS_REQUIREMENT_TYPE.AT_LEAST_ONE,
    description: () => `${formatX(1e11)} multiplier on all Infinity Dimensions,boosted by total EC tiers completed`,
    effect: () => Math.pow(1e11,1+0.1*EternityChallenges.completions**0.7) ,
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 171,
  ...
  },
  {
    ...
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: () => `You gain 10% of your Infinity Points gained on crunch each second`,
    effect: () => gainedInfinityPoints().times(Time.deltaTime / 10)
      .timesEffectOf(Ra.unlocks.continuousTTBoost.effects.autoPrestige)
  },
  {
    id: 191,
    cost: 400,
    requirement: [181, () => EternityChallenge(10).completions > 0 || Perk.studyPassive.isBought],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: () => `Antimatter Dimension multiplier based on Banked Infinities`,
    effect: () => (DC.E16500.pow(Currency.infinitiesBanked.value.div(1e9).clampMax(1))),
    cap: DC.E16500,
    formatEffect: value => formatX(value, 2, 1),
  },
  {
    id: 192,
    cost: 730,
    requirement: [181, () => EternityChallenge(10).completions > 0 || Perk.studyPassive.isBought, () => !Enslaved.isRunning],
    reqType: TS_REQUIREMENT_TYPE.ALL,
  ...
  },
  {
    ...
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: "Dilated Time multiplier based on TG",
    effect: () => player.dilation.totalTachyonGalaxies**1.3+1,
    formatEffect: value => formatX(value, 2, 2)
  },
  {
    id: 242,
    ...
    requirement: [() => PlayerProgress.dilationUnlocked()],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: "Time theorem generation boosts itself",
    effect: () => Math.max(player.timestudy.theorem.log10(),1)**1.5,
    formatEffect: value => formatX(value, 2, 2)
  },
  {
    id: 251,
    ...
    requirement: [241],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: "DT boost based on log10(EP)^0.6",
    effect: () => Math.max(Currency.eternityPoints.value.log10(),1)**0.6,
    formatEffect: value => formatX(value, 2, 2)
  },
  {
    id: 252,
    ...
    requirement: [242],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: "Further reduce dilation penalty (^1.05)",
    effect:  1.05,
  },
  {
    id: 261,
    ...
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: "Perkpoints bonus based on log10(EP)^0.1",
    effect: () => Math.max(Currency.eternityPoints.value.log10(),1)**0.1,
    formatEffect: value => formatX(value, 2, 2)
  },
  {
    id: 271,
    cost: 1e90,
    requirement: [261, () => TeresaUnlocks.effarig.isUnlocked],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: () => (TeresaUnlocks.effarig.isUnlocked
      ? `Generate 10% of Relic shards gained on Reality every IRL second.`
      : `Unlock Giraffe to reveal the study.`),
    effect: () => Effarig.shardsGained * 0.1,
  },
  {
    id: 272,
    cost: 1e36,
    requirement: [261, () => RealityUpgrade(19).isBought],
    reqType: TS_REQUIREMENT_TYPE.ALL,
    description: () => (RealityUpgrade(19).isBought
