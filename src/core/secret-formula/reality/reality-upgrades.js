...
export const realityUpgrades = [
  ...
  },
  {
    ...
    requirement: () => `Eternity for ${format("1e4000")} Eternity Points using
      only less than 3 Glyphs.`,
    hasFailed: () => {
      const invalidEquippedGlyphs = Glyphs.activeWithoutCompanion.length > 3 ;
  ...
  },
  {
    ...
    description: () => `Start every Reality with ${formatInt(110000)} Eternities (also applies to current Reality)`,
    automatorPoints: 15,
    shortDescription: () => `Start with ${formatInt(110000)} Eternities`,
    effect: () => 110000
  },
  {
    ...
    description: "Every second, gain 50% of the Infinities you would normally gain by Infinitying",
    automatorPoints: 5,
    shortDescription: () => `Continuous Infinity generation`,
    effect: () => gainedInfinities().times(0.5),
  ...
  },
  {
    ...
    checkEvent: [GAME_EVENT.ETERNITY_RESET_AFTER, GAME_EVENT.REALITY_FIRST_UNLOCKED],
    description: "Gain Eternities per second equal to your Reality count times 10",
    automatorPoints: 5,
    shortDescription: () => `Continuous Eternity generation`,
    effect: () => Currency.realities.value * 10 * Ra.unlocks.continuousTTBoost.effects.eternity.effectOrDefault(1),
    formatEffect: value => `${format(value)} per second`
  },
  {
  ...
  },
  {
    ...
    id: 16,
    cost: 1500,
    requirement: () => `Reality with more than or ${formatInt(1)} Glyph equipped of uncommon or better rarity
    ...
    hasFailed: () => {
      ...
      return equipped + Math.min(availableGlyphs, availableSlots) < 1;
    },
    checkRequirement: () => Glyphs.activeWithoutCompanion.countWhere(g => g.strength >= 1.5) >= 1,
  ...
  },
  {
    ...
    id: 17,
    cost: 1500,
    requirement: () => `Reality with more than or ${formatInt(1)} Glyph equipped, each having at least ${formatInt(2)} effects
    ...
    checkRequirement: () => Glyphs.activeWithoutCompanion.countWhere(g => countValuesFromBitmask(g.effects) >= 2) >= 1,
    checkEvent: GAME_EVENT.REALITY_RESET_BEFORE,
    description: () => `always get an additional effect on Glyphs`,
    effect: 1,
    formatCost: value => format(value, 1, 0)
  },
  {
    ...
    id: 18,
    cost: 1500,
    requirement: () => `Reality with more than or ${formatInt(1)} Glyph equipped at level ${formatInt(3)} or higher
      (${formatInt(Glyphs.activeWithoutCompanion.countWhere(g => g && g.level >= 10))} equipped)`,
    hasFailed: () => {
      const availableGlyphs = Glyphs.inventory.countWhere(g => g && g.level >= 3);
      const equipped = Glyphs.activeWithoutCompanion.countWhere(g => g.level >= 3);
      const availableSlots = Glyphs.activeSlotCount - Glyphs.activeList.length;
      return equipped + Math.min(availableGlyphs, availableSlots) < 1;
    },
    checkRequirement: () => Glyphs.activeWithoutCompanion.countWhere(g => g.level >= 10) >= 1,
    checkEvent: GAME_EVENT.REALITY_RESET_BEFORE,
  ...
  },
  {
    ...
    id: 19,
    cost: 1500,
    requirement: () => `Have a total of ${formatInt(1)} or more Glyphs at once
      (You have ${formatInt(Glyphs.allGlyphs.countWhere(g => g.type !== "companion"))})`,
    hasFailed: () => Glyphs.allGlyphs.countWhere(g => g.type !== "companion") < 0,
    checkRequirement: () => Glyphs.allGlyphs.countWhere(g => g.type !== "companion") >= 0,
    checkEvent: GAME_EVENT.REALITY_RESET_BEFORE,
