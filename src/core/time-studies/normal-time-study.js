...
NormalTimeStudies.pathList = [
  ...
  { path: TIME_STUDY_PATH.CELESTIAL, studies: [271], name: "Celestial" },
  { path: TIME_STUDY_PATH.REALITY, studies: [272], name: "Reality" }
];
...
export class NormalTimeStudyState extends TimeStudyState {
  constructor(config) {
    const type = (config.id > 300 && config.id < 305) ? TIME_STUDY_TYPE.TRIAD : TIME_STUDY_TYPE.NORMAL;
    super(config, type);
  ...
  get isTriad() {
    return (this.id > 300 && this.id < 305);
  }
  ...
  purchase(auto = false) {
    ...
    if (this.id > 300 && this.id < 305) player.requirementChecks.reality.noTriads = false;
