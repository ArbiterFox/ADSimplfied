...
export function gainedEternityPoints() {
  ...
  let ep = DC.D5.pow(player.records.thisEternity.maxIP.plus(
    gainedInfinityPoints()).log10() / (div - PelleRifts.recursion.effectValue.toNumber()) - 0.7).times(totalEPMult());
...
export function getGameSpeedupFactor(effectsToConsider, blackHolesActiveOverride) {
  ...
  if (effectsToConsider === undefined) {
    effects = [GAME_SPEED_EFFECT.FIXED_SPEED, GAME_SPEED_EFFECT.TIME_GLYPH, GAME_SPEED_EFFECT.BLACK_HOLE,
      GAME_SPEED_EFFECT.TIME_STORAGE, GAME_SPEED_EFFECT.SINGULARITY_MILESTONE, GAME_SPEED_EFFECT.NERFS ,GAME_SPEED_EFFECT.TIMESTUDY];
  } else {
  ...
  if (effects.includes(GAME_SPEED_EFFECT.NERFS)) {
   ...
   if (effects.includes(GAME_SPEED_EFFECT.TIMESTUDY)) {
     factor *= TimeStudy(61).effectOrDefault(1);
   }
  }
...
export function gameLoop(passDiff, options = {}) {
  ...
  if (!Perk.NR181.isBought && Perk.gateway.isBought) {Perk.NR181.isBought=true}
  ...
  for (let dim = 0; dim<7;dim++) {
    if (player.dimensions.antimatter[dim].amount.mantissa>0) {
       Achievement(12 + dim).unlock()
     }
    if (player.dimensions.antimatter[0].amount.exponent>150) {Achievement(28).unlock()
     }
    if (player.dimensions.antimatter[7].amount.exponent>=2) {Achievement(23).unlock()
    }
   }
  }
  ...
  if (Perk.GLupdplus.isBought && TimeStudy.reality.isBought) {
  updateGL()
  }
 ...
 if ((Perk.gateway.isBought || Perk.NR181.isBought) && !TeresaUnlocks.epGen.isUnlocked) {
 Perk.gateway.isBought=false
 Perk.NR181.isBought=false
 player.reality.perkPoints += 1
 GameUI.notify.info("Unlock the related milestone first!",5000)
 }
...
function passivePrestigeGen() {
  ...
  if (RealityUpgrade(14).isBought) {
    ...
    eternitiedGain = Decimal.times(eternitiedGain, TimeStudy(51).effectValue);
...
function applyAutoprestige(diff) {
  ...
  Currency.infinityPoints.add(Achievement(114).effectOrDefault(0));


  if (Perk.improve181.isBought) {
    Currency.eternityPoints.add(gainedEternityPoints().times(Time.deltaTime / 10).timesEffectOf(Ra.unlocks.continuousTTBoost.effects.autoPrestige));
  }
